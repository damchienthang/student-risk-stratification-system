...
for name, model in models.items():
    print(f"\n--- Training {name} ---")
    if name in ['XGBoost']:
        model.fit(X_train_resampled, y_train_resampled) # SMOTE with XGB
    else:
        model.fit(X_train_scaled, y_train) # Others use class_weight='balanced'
    ...
    print(f"Validation F1-Macro: {macro_f1:.4f}")
    results[name] = {'model': model, 'f1_macro': macro_f1, 'acc': acc, 'b_acc': b_acc}
...
print("\nSo sánh các mô hình:")
print(df_results)
df_results.plot(kind='bar', y=['f1_macro', 'b_acc'], figsize=(10,6), colormap='viridis')
plt.title('So sánh hiệu suất các mô hình trên tập Validation')
plt.ylabel('Score')
plt.ylim([0, 1])
plt.xticks(rotation=0)
plt.legend(['F1 Macro', 'Balanced Accuracy'])
plt.tight_layout()
