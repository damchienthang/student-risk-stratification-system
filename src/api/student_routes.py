...
async def student_dashboard(request: Request):
    ...
    try:
        ...
        # External students might have non-numeric usernames
        return templates.TemplateResponse("index.html", {
            "request": request,
            "error": "Chế độ Dashboard chỉ dành cho sinh viên OULAD.",
            "user": user
        })
    ...
    if not student:
        return templates.TemplateResponse("index.html", {
            "request": request,
            "error": f"Không tìm thấy thông tin sinh viên #{student_id}.",
            "user": user
        })
    ...
    # Predict to get recommendations and confidence
